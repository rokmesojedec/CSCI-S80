...
def load_data(data_dir):
    ...
    for root, subdirectories, files in os.walk(data_dir):
        ...
        # extract catgory from subdirectory folder name
        if len(category) == 2:
            category = category[1]
        else:
            continue
        for file in files:
            ...
            # if file extension is ppm
            # process image and add it to sample array
            if file_extension == "ppm":
                image = cv2.imread(os.path.join(root, file))
                resized = cv2.resize(image, (IMG_WIDTH, IMG_HEIGHT))
                img_labels[0].append(resized)
                img_labels[1].append(int(category))
    return img_labels
